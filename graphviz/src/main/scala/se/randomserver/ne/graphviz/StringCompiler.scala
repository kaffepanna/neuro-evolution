...
object StringCompiler:
  ...
  def compiler(level: Int = 0): GrammarA ~> GraphVizState =
    new (GrammarA ~> GraphVizState) {
      ...
      override def apply[A](fa: GrammarA[A]): GraphVizState[A] = fa match
        ...
        case GrammarA.GSubGraph(name, g) =>
          for {
