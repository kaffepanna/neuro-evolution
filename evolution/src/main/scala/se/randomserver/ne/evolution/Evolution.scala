...
object Evolution {
  ...
  def modifyState[F[_], W, S, B](
      fn: EvolutionState[W, S] => F[(EvolutionState[W, S], B)]
  )(using Monad[F], HasEvolutionState[F, W, S]): F[B] = for
  ...
  def getState[F[_], W, S](using
      hs: HasEvolutionState[F, W, S]
  ): F[EvolutionState[W, S]] = hs.get
  def setState[F[_], W, S](using hs: HasEvolutionState[F, W, S]) = hs.set
  def getEnv[F[_], W, S](using
      he: HasEvolutionEnv[F, W, S]
  ): F[EvolutionEnv[W, S]] = he.ask

  def nextSpeciesId[F[_], W, S](using
      Monad[F],
      HasEvolutionState[F, W, S]
  ): F[SpeciesId] = modifyState { (state: EvolutionState[W, S]) =>
    val nextId = state.nextSpeciesId + 1
    (state.copy(nextSpeciesId = nextId) -> nextId).pure
  }

  def nextGenomeId[F[_], W, S](using
      Monad[F],
      HasEvolutionState[F, W, S]
  ): F[GenomeId] = modifyState { (state: EvolutionState[W, S]) =>
    val nextId = state.nextGenomeId + 1
    (state.copy(nextGenomeId = nextId), nextId).pure
  }

  def evaluateFitness[F[_]: Monad, W: Semiring, S: Order](using
      HasEvolutionEnv[F, W, S],
      HasEvolutionState[F, W, S]
  ): F[Unit] = for {
    env <- getEnv[F, W, S]
    expected = env.data.map(_._2)
    state <- HasEvolutionState[F, W, S].get
    scores = state.population.map { case (id, genome) =>
      ...
      val outputs = env.data.map(_._1.toVector).map { input =>
        ...
        Runner
          .evalNetwork(compiled, in, env.defaultBias, env.recurrentSteps)
          .toList
      }
  ...
  def adjustFitnessSharing[F[_]: Monad, W, S: Field](using
      HasEvolutionState[F, W, S]
  ): F[Unit] = for {
      ...
      val speciesSize = state.species.find(_.members.contains(genomeId)) match {
        ...
        case None          =>
          throw new IllegalStateException(
            s"Genome: $genomeId not member of any species"
          )
      }
  ...
  def cullSpecies[F[_]: Monad, W, S: Order](using
      HasEvolutionEnv[F, W, S],
      HasEvolutionState[F, W, S]
  ): F[Unit] = for {
    env <- getEnv[F, W, S]
  ...
  def allocateOffspringPerSpecies[F[_]: Monad, W, S: Numeric: Order](using
      HasEvolutionEnv[F, W, S],
      HasEvolutionState[F, W, S]
  ): F[Map[SpeciesId, Int]] =
    for {
      ...
      speciesFitnesses = state.species.map { species =>
        val fitnessSum = species.members
          .map(state.adjustedFitness)
          .foldLeft(Numeric[S].zero)(_ + _)
      ...
      shifted =
        if (minFitness <= Numeric[S].zero) speciesFitnesses.map {
          case (id, f) => id -> (f - minFitness)
        }
  ...
  def reproduce[F[_]: Monad, W: Semiring: Order, S: Order](
    ...
    children <- state.species.flatMap { species =>
      ...
      val eliteCount =
        math.max(1, (species.members.size * env.eliteFraction).ceil.toInt)
      val elites =
        sortedMembers.take(eliteCount).map(id => id -> state.population(id))
      ...
      if (count <= 0)
        ...
        val children = Vector.fill(childrenCount) {
          for {
            ...
            (id2, p2) = id2p2
            (dominant, recessive) =
              if (state.adjustedFitness(id1) >= state.adjustedFitness(id2))
                (p1, p2)
              else (p1, p2)
            child <-
              cross[F, W](dominant, recessive) >>= mutate[F, W](
                env.weightChance,
                env.connectionChance,
                env.nodeChance,
                env.resetChance
              )
  ...
  def speciate[F[_]: Monad, W: Field: Signed: Order, S](
      ...
      newPopulation.foldLeft((clearedSpecies, Vector.empty[GenomeId])) {
        case ((speciesAcc, unassignedAcc), (genomeId, genome)) =>
          speciesAcc.indexWhere { s =>
            genome.compare(
              state.population(s.representative),
              env.speciationConfig
            ) < env.speciationConfig.threshold
          } match {
            case -1 =>
    ...
    _ <- setState[F, W, S](
      state.copy(
        ...
        nextSpeciesId = state.nextSpeciesId + clusteredNewSpecies.size,
  ...
  def reassignRepresentatives[F[_]: Monad, W, S: Order: Semiring](using
      HasEvolutionState[F, W, S],
      Random[F]
  ): F[Unit] =
    for {
  ...
  def debug[F[_]: Monad: Random, W: Order, S: Semiring: Order](using
      HasEvolutionState[F, W, S]
  ): F[Unit] = for {
    state <- getState[F, W, S]
    ...
    _ = {
      ...
      state.species.foreach { species =>
        val avgSize = species.members
          .map(state.population)
          .foldLeft(0)((n, genome) =>
            n + genome.nodes.size
          ) / species.members.size
        val avgConnections = species.members
          .map(state.population)
          .foldLeft(0)((n, genome) =>
            n + genome.genes.size
          ) / species.members.size
        println(
          s"\tSpecies: ${species.id} - age: ${species.age}, size: ${species.members.size}, score: ${String.format("%.2f", speciesFitnesses(species.id))}, nodes: ${avgSize}, conns: ${avgConnections}"
        )
      }
    }
  } yield ()
  ...
  def evolveStep[
      F[_]: Monad: Random,
      W: Field: Signed: Semiring: Order,
      S: Field: Numeric: Semiring: Order
  ](using
      RandomRange[F, W],
      HasEvolutionEnv[F, W, S],
      HasEvolutionState[F, W, S],
    ...
    _ <- modifyState[F, W, S, Unit] { state =>
      val ageIncremented =
        state.species.map[Species[W]](s => s.copy(age = s.age + 1))
      (state.copy(
