...
class ChartViewModel(val session: SessionViewModel) {
  val seriesMap: ObservableHashMap[SpeciesId, JFXSeries[Number, Number]] =
    ObservableHashMap[SpeciesId, JFXSeries[Number, Number]]()
  ...
  session.speciesFitness.onChange { (_, change) =>
    change match
      case ObservableMap.Add(generation, fitness) =>
        fitness.foreach { (species, score) =>
          val serie = seriesMap.getOrElseUpdate(
            species,
            Series[Number, Number](s"$species", ObservableBuffer())
          )
          serie.data() += Data(generation, score)
        }
      case ObservableMap.Remove(generation, _) =>
        seriesMap.foreach { (seriesId, serie) =>
