...
object GameEvolution {
  ...
  given [F[_]](using
      AP: Applicative[F],
      GEE: HasGameEvolutionEnv[F]
  ): HasEvolutionEnv[F, Double, Double] =
    new Ask[F, EvolutionEnv[Double, Double]] {
      override def applicative: Applicative[F] = AP
      override def ask[E2 >: EvolutionEnv[Double, Double]]: F[E2] =
        GEE.ask.map(_.evolutionEnv)
    }

  given [F[_]](using
      M: Monad[F],
      GES: Stateful[F, GameEvolutionState]
  ): HasEvolutionState[F, Double, Double] =
    new Stateful[F, EvolutionState[Double, Double]] {
      override def monad: Monad[F] = M
      override def get: F[EvolutionState[Double, Double]] =
        GES.get.map(_.evolutionState)
      override def set(es: EvolutionState[Double, Double]): F[Unit] =
        GES.modify(gs => gs.copy(evolutionState = es))
      override def modify(
          f: EvolutionState[Double, Double] => EvolutionState[Double, Double]
      ): F[Unit] =
        GES.modify(gs => gs.copy(evolutionState = f(gs.evolutionState)))
    }

  type GameEvo[F[_]] = Monad[F] & HasGameEvolutionState[F] &
    HasGameEvolutionEnv[F] & StatsCallback[F] & HasGenePool[F]

  def getGameEnv[F[_]: HasGameEvolutionEnv]: F[GameEvolutionEnv] =
    Ask[F, GameEvolutionEnv].ask
  ...
  def integrate[F[_]: Monad: HasGameEvolutionEnv: Parallel](
    ...
    case nn =>
      for {
        ...
        state = acc.last
        intents <- agents
          .map { case (id, (team, member)) =>
            val vision = Game.vision(state, id, env.visionRadius)
            val inputs = vision.flatten
              .map {
              ...
              .zip(member.inputs.toVector.sorted)
              .map(_.swap)
              .toMap

            val activationState = activationStates(id)
            val nextActivationState =
              Runner.stepNetwork(member, inputs, 1.0, activationState)
            val intent = member.outputs.toVector.sorted
              .map(nextActivationState.apply)
              .zip(Game.Action.values)
              .maxBy(_._1)
        ...
        stuck = (acc :+ nextGameState).reverse
          .take(5)
          .map(a => a.individuals.values.map(_.score).sum)
          .toSet
          .size == 1
        res <-
  ...
  def gameStep[F[_]: Monad: Applicative: Parallel: HasGameEvolutionEnv](
  ...
  ): F[(Vector[GameState], Map[GenomeId, Double])] = for {
    gameEnv <- getGameEnv
    shuffled <- Random[F].shuffleVector(members.toVector)
    teams = Utils
      .splitEvenly(shuffled, gameEnv.teams)
      .zipWithIndex
      .flatMap { case (a, team) =>
        a.map { case (id, m) =>
          (id, (team, m))
    ...
    initailActivationStates = Map
      .empty[GenomeId, ActivationState[Double]]
      .withDefault { i =>
    ...
    states <- integrate(
      gameEnv.gameIterations,
      teams,
      Vector(initialGameState),
      initailActivationStates
    )
  ...
  def evaluateFitness[F[
    ...
    updatedFitness = runs
      .map(_._2)
      .foldLeft(Map.empty[Int, (Double, Int)]) { (acc, map) =>
  ...
  def pushStats[F[_]: Monad: HasGameEvolutionState: StatsCallback](
      runs: Vector[Vector[GameState]]
  ): F[Unit] = for {
    state <- getState
  ...
  def step[
      F[_]: Monad: Parallel: HasGameEvolutionEnv: HasGameEvolutionState: HasGenePool: StatsCallback
  ](using
      R: Random[F],
    ...
    _ <- setState({
      val ageIncremented =
        state.species.map[Species[Double]](s => s.copy(age = s.age + 1))
      state.copy(
        generation = state.generation + 1,
        species = ageIncremented,
        fitness = Map.empty,
        adjustedFitness = Map.empty
      )
    })
  } yield runs.last

  def evolve[
      F[_]: Monad: Random: Parallel: HasGameEvolutionEnv: HasGameEvolutionState: HasGenePool: StatsCallback
  ](using
    ...
    genomes <- genome(env.inputs, env.outputs, env.initialHidden).replicateA(
      env.popsize
    )
  ...
  def run(
      env: GameEvolutionEnv
  )(using StatsCallback[IO], RandomRange[IO, Double], Random[IO]) = {
    ...
    for {
      gameEvolutionStateRef <- Ref.of[IO, GameEvolutionState](
        GameEvolutionState(EvolutionState[Double, Double]())
      )
      genePoolStateRef <- Ref.of[IO, GenePool] { GenePool(0, Map.empty) }
      _ <- {
        given HasGameEvolutionEnv[IO] = Ask.const[IO, GameEvolutionEnv](env)
        given HasGameEvolutionState[IO] =
          refToStateful[GameEvolutionState](gameEvolutionStateRef)
