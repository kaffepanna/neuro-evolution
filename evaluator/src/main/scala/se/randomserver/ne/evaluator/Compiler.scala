...
object Compiler {
  ...
  def compileGenomeToBlockGraph(genome: Genome[?]): BlockGraph = {
    ...
    val sccs: Vector[SCC[Int]] =
      Graph.tarjanSCC(genome.nodes.toSet, edges.apply).toVector
    val nodeToBlock: Map[Int, BlockId] = sccs.zipWithIndex.flatMap {
      case (scc, id) =>
        scc.map(_ -> id)
    }.toMap
    ...
    val blockEdges: Map[BlockId, Set[BlockId]] =
      genome.genes
        .filter(_.enabled)
        .foldLeft(
          Map.empty[BlockId, Set[BlockId]].withDefaultValue(Set.empty)
        ) { case (acc, g) =>
    ...
    val sortedBlockIds: Seq[BlockId] =
      Graph.kahnTopoSort(nodeToBlock.values.toSet, blockEdges.apply)
  ...
  def compileGenome[W](
    ...
    def compileBlock(block: Block): CompiledBlock[W] = {
      ...
      val isRecurrent =
        block.nodes.size > 1 ||
          enabledGenes.exists(g =>
            g.from == g.to && block.nodes.contains(g.from)
          )
